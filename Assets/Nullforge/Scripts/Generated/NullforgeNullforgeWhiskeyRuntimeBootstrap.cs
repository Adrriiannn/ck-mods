...
public sealed class NullforgeNullforgeWhiskeyRuntimeBootstrap : IMod
{
  private const string DimensionId = "Nullforge:Whiskey";
  private const string DimensionDisplayName = "Whiskey";
  private const string ContentPackId = "Nullforge:WhiskeyPack";
  private const string ContentPackDisplayName = "Whiskey Pack";
  ...
  private const string EntryPortalId = "Nullforge:Whiskey.portal.entry";
  private const string ReturnPortalId = "Nullforge:Whiskey.portal.return";
  private const string EntryPresentationId = "Nullforge:Whiskey.portal.entry.presentation";
  private const string ReturnPresentationId = "Nullforge:Whiskey.portal.return.presentation";
  private const string PortalObjectName = "Nullforge_NullforgeWhiskey_Portal";
  private const string ReturnPortalObjectName = "Nullforge_NullforgeWhiskey_Portal_Return";
  private const string PortalDisplayName = "Nullforge Portal";
  private const string PortalDescription = "Portal to Whiskey.";
  private const string EntryFromDimensionId = "corekeeper.overworld";
  private const string EntryToDimensionId = "Nullforge:Whiskey";
  private const string ReturnFromDimensionId = "Nullforge:Whiskey";
  ...
  private const string StarterId = "Nullforge:Whiskey.starter";
  ...
  {
    // Custom tilesets register the moment their asset loads — mod load runs before
    // the ECS worlds are created, so rendering, placement and the map-color table
    // are all wired before any tile of the set can appear.
    DimensionTilesetAsset tilesetAsset = obj as DimensionTilesetAsset;
    if (tilesetAsset != null)
    {
      DimensionTilesetAssetRuntime.Register(tilesetAsset);
      return;
    }
  ...
  private void EnsureStaticRuntimeExtras()
  {
    ...
    DimensionPortalSoundRegistry.Register(
        ...
        string.Empty);
    DimensionPortalSoundRegistry.Register(
        "Nullforge_NullforgeWhiskey_Portal_Instant",
        0,
        "AF_portal_appear",
        "AF_portal_collapse",
        "");
    DimensionCraftingRegistry.Register(
        new DimensionCraftingRecipeDefinition(
            "Nullforge:Nullglass.wall.block",
            ObjectID.WoodenWorkBench,
            1,
            0f,
            "Nullglass Block"));
  ...
  private bool EnsureMinimumGenerationPasses(IDimensionService current)
  {
    if (!TryEnsureGenerationPass(
        ...
        new DimensionGenerationPassDefinition(
            "Nullforge:Whiskey.single-biome.terrain-pass",
            "Terrain",
            "Nullforge:Whiskey",
            "Nullforge:Iceborne",
            true,
