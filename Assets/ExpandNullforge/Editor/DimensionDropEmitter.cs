...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static class DimensionDropEmitter
    {
        ...
        /// being written, so <see cref="ApplyCustomLoot"/> can only skip such an entry. The drop
        /// router used to send those here anyway — the bootstrap emitter skipped them because "it
        /// was already written onto the prefab" and the prefab write skipped them because the name
        /// resolved to nothing, so the drop was written NOWHERE. Failing the fit here is what sends
        /// it down the runtime road instead, which resolves names at load and already works.
        /// </para>
        /// </remarks>
        ...
        /// </para>
        /// <para>
        /// It used to take the highest and write that for all of them, which made a one-in-ten item
        /// drop nine times in ten with nothing said. Now the object carries the chance the most
        /// drops asked for — highest wins a tie — and every drop that asked for a different one is
        /// sent to the source's loot table instead, where a chance of its own can be made real.
        /// </para>
        /// </remarks>
            ...
            Action<string> report = null)
        {
            ...
            if (loot.hasLootTable)
            {
                ...
                // prefab it wrote last time, and nothing used to set hasLootTable back to false, so
                // a creature given the game's AncientChest table once kept it after the field was
                // cleared — and every drop authored against that creature was appended to the
                // game's shared ancient-chest table.
                //
                // All three generators that reach here now call ClearAnyLootTheLastGenerateWrote
        ...
        /// <remarks>
        /// <para>
        /// THE INVARIANT USED TO BE TRUE OF ONE GENERATOR IN THREE. The earlier take-back had a
        /// single caller (it has since been deleted, having none at all), and the two that did not
        /// call it — chests and world objects — both
        /// reload the prefab they wrote last time and both skipped their whole loot pass when the
        /// drop list was empty. So deleting every drop off a chest left last time's drops baked on
        /// it, and renaming the mod left the prefab pointing at a table minted from the OLD name
        /// while the runtime filled one minted from the new: table drops stopped, and the report
        /// said the object generated fine.
        /// </para>
        /// <para>
        ...
        /// has to read the id back off a prefab entity — which is what it used to do, through a
        /// database lookup that cannot answer while the database is being converted.
        /// </remarks>
        ...
        /// <remarks>
        /// <para>
        /// The reason used to be given as "an amount range or a biome restriction", which stopped
        /// being the whole truth the moment a drop of this mod's own item started coming down here
        /// as well — that one asks for neither. A reason that does not apply is worse than none: it
        /// sends somebody looking for a range they never typed.
        /// </para>
        /// <para>
        /// It also used to end "so nothing is lost", which is not true of two always-drops entries
