...
namespace ExpandNullforge.EditorTools
{
    ...
    internal sealed class DimensionLayoutStudio
    {
            ...
            DimensionLayoutTemplateAsset layout)
        {
            ...
            if (radial)
            {
                // "Band size" used to be edited here, described as how finely each ring is cut into
                // blocks. The compiler reads it, hands it to BuildRadialRingBands and the body of
                // that method never mentions it: the scanline walk that replaced banding writes an
                // exact circle. The stored value stays, because every pinned layout fingerprint
                // already includes it.
                EditorGUILayout.LabelField(
