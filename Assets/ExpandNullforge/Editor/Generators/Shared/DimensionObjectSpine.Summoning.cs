...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static partial class DimensionObjectSpine
    {
        ...
        /// <remarks>
        /// <para>
        /// WITHOUT THIS A GENERATED CIRCLE DOES NOTHING AT ALL. Core Keeper's
        ...
        /// and it carries no <c>[RequireComponent]</c>, so a circle built from it alone is never in
        /// the query. Everything upstream still succeeds — the boss id resolves, the item is
        /// wired, the arena check passes — and the idol placed on the circle does nothing, forever,
        ...
        public static void MakeTheCircleNoticeTheItem(GameObject root, float looksForItsItemWithin)
        {
            ...
            // THE THREE FIELDS ARE MERGED, NOT SET OUTRIGHT, because on a world object setting them
            // is a clobber. The tracker is [DisallowMultipleComponent] — one reach and one layer
            // mask shared by every answer on the object that watches its surroundings — and a
            // circle is usually not the only such answer. Running last and setting them replaces a
            // shove authored at six tiles with 1.7 and narrows the mask to one layer, silently.
            // Merging keeps both features:
