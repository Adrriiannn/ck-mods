...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static class DimensionGeneratedObjectIds
    {
        ...
        private static List<string> Walk(DimensionTemplateAsset template, bool wantEnabled)
        {
            ...
            //
            // IT READS THE TILESET'S OWN SWITCH, like every other branch. Skipping the
            // SwitchedOff walk and never looking at Enabled lands an unticked tileset's block in
            // Collect: naming.Owns says yes, NamesSomethingSwitchedOff cannot fire
            // (it needs !Owns), the reference defers, a row is written, and the game spends the
            // session retrying a name that is never coming. The item generator gates
    ...
    internal sealed class DimensionObjectBinder
    {
        ...
        /// </summary>
        /// <remarks>
        /// Passing null here leaves every generator's binder able to tell only "ours" from "not a
        /// name at all" — and a reference to the mod's own unticked projectile then gets the
        /// sentence for a misspelling. The context carries the set, so the generators and the
        /// bootstrap emitter say the same thing about the same reference.
        /// </remarks>
