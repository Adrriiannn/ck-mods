    ...
    internal static partial class DimensionObjectSpine
    {
            ...
            System.Func<string, bool> isDeferred = null)
        {
            ...
            if (weapon.IsRanged)
            {
                ...
                // SAID FOR BOTH HALVES OF "names nothing". Requiring the field to be EMPTY as well
                // loses the misspelling half — a wound-up shot naming something the game does not
                // have — without a word, while the message above talks only about the shot. Both
                // roads end with the size at zero, so both have to say the size went.
        ...
        /// the dust the last generate gave it. An unknown puff name is reported rather than dropped, because a
