            ...
            System.Func<string, bool> isDeferred = null)
        {
            ...
            if (loot.hasLootDropsOnTakingDamage)
            {
                ...
                else if (root.GetComponent<HealthAuthoring>() == null)
                {
                    ...
                    // that says what to do. The check covers every target, not only one of our own:
                    // narrowed, it leaves the common case hitting the game's error with nothing said.
                    loot.hasLootDropsOnTakingDamage = false;
                    if (report != null)
                    {
                        report(
            ...
            if (loot.hasLootDropsOnUse)
            {
                ...
                // THE TABLE IS WRITTEN EVERY TIME, THE CLEARED ONE INCLUDED. Written only inside
                // the guard, with no else, an author who sets a table, generates, then clears the
                // table while keeping items in "gives on use" goes on handing out the old table
                // forever. A misspelling takes the same road and says nothing.
