...
namespace ExpandNullforge.EditorTools
{
    ...
    /// <remarks>
    /// <para>
    /// THE WHOLE CHAIN, BECAUSE HALF OF IT IS NOT ENOUGH. A vehicle written as
    /// <c>ObjectType.NonObtainable</c> with no placement is one nothing can hold, put
    /// down or break; it exists only where a scene happens to spawn it, and its name reaches
    /// nobody. Everything below closes one link of that chain, and each link was checked
    ...
    internal static class DimensionVehicleGenerator
    {
            ...
            Action<string> say)
        {
            ...
            {
                // A vehicle that named one of the mod's blocks on an earlier generate and does not
                // now must not
