    ...
    internal static partial class DimensionCreatureGenerator
    {
            ...
            DimensionCreatureGenerationReport report)
        {
            ...
            else
            {
                ...
                // A name that is neither the game's nor one of ours is reported here rather than
                // carried. Carried, it ships the egg with a qualified nonsense name that says
                // nothing until the game gives up on it hours later.
            ...
            DimensionCreatureGenerationReport report)
        {
            ...
            // THE TAKE-BACK HAS TO RUN ABOVE THE EARLY RETURN. Below it, a creature that has its
            // loot table and its drops cleared but still sheds ore keeps last generate's table
            // stamped on it — and the "you are using the game's own table" warning then names a
            // table the author has already deleted.
            ...
            // death loot table is the normal shape of a pet mob, so below the return a creature
            // with no loot silently never becomes a pet and never carries its extra loot, with no
            // warning anywhere.
            ...
            // creatures are authored without one. Given none and left alone it carries no
            // DropsLootFromLootTableCD at all, so the drop has nowhere to be registered at load
