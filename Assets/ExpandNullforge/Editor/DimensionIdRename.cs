...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static class DimensionIdRename
    {
        ...
        private static void AddNameBlockers(DimensionIdRenamePlan plan, string newId)
        {
            ...
            //
            // ASKED OF THE LOCAL HALF, which is the whole point of the check. It used to be asked
            // of the string as typed, and ids are created qualified by default, so the one form a
            // creator actually types — "MyMod:CopperShovel" — walked straight past a refusal that
            // caught only "CopperShovel". Owns() keys on the local half and rejects anything
            // vanilla owns, so the qualified form breaks in exactly the same way.
            //
        ...
        /// </para>
        /// <para>
        /// THE FORM IS PRESERVED IN BOTH DIRECTIONS. It used to be preserved only one way: the
        /// qualified branch put the row's own qualifier back, while the plain branch wrote the new
        /// name exactly as it was typed — so typing "MyMod:Sword" over "Blade" put a qualifier into
        /// a field that had never carried one. A field written unqualified stays unqualified, which
        /// is what the class remarks have always said this does.
        /// </para>
        /// <para>
