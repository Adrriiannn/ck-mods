...
namespace ExpandNullforge.EditorTools
{
    ...
    internal sealed class DimensionObjectLinkEndToEndTests
    {
        ...
        {
            // ForOutputFolder used to own nothing, so QualifyReference was the identity function:
            // the station shipped "EmberBolt" while the item had registered as "MyMod:EmberBolt",
            // the lookup missed, and the station crafted nothing.
        ...
        public void AModDropThatWouldFitAsCustomLootGoesToTheRuntimeRegistryInstead()
        {
            ...
            // onto the prefab at all. The router used to send it there anyway and the bootstrap
            // skipped it as "already written", so the drop landed nowhere.
        ...
        public void AModCreatureDroppingAModItemShipsTheTableItWasGivenToHangDropsOn()
        {
            ...
            // the mod's own has somewhere to drop from. At load the injection used to read that
            // table back off the creature's prefab ENTITY, through PugDatabase — inside a Harmony
            // prefix on LootTableConverter.Convert, which runs during database conversion, when
            // Manager.ecs has no world to answer with. The read failed every time, the drop was
            // reported as belonging to a creature with no loot table, and the pass latched.
            //
            ...
            // whole test survived the stamp being taken out of ApplyLoot, which is the one write
        ...
        public void AnAuthoredGameLootTableOnAModCreatureIsSaidRatherThanUsedQuietly()
        {
            ...
            // THE SAYING, DRIVEN. This used to assert two pure functions and nothing else, so
            // deleting the whole reporting block left it green under a name that promises the
            // opposite.
