...
namespace ExpandNullforge.EditorTools
{
    ...
    /// </list>
    /// <para>
    /// SO THE GAP IT LEAVES IS DISCOVERY, and it is two hundred components wide. Four
    /// censuses on 2026-08-29 read 186 of those two hundred against the system that consumes
    /// them, and those names live in
    ...
    internal sealed partial class DimensionQueryCompanionTests
    {
        ...
        /// runtime <c>*CD</c> components. The authoring suffix alone leaves a
        /// new helper used only on a runtime component — <c>Attach&lt;CavelingCD&gt;(entity)</c> —
        /// matching neither this nor the add scan, so it ships with nobody answering
        ...
        /// one that writes a surface, so it needs no sweep either. So every name here has to be a
        /// form the tree really uses, and nine plausible ones are deliberately absent —
        ...
        /// this tree.
        /// <see cref="EveryNameExcusedFromAddingIsAFormTheTreeReallyUses"/> keeps the claim
        ...
        public void EveryFileThatWritesASurfaceEitherSweepsOrSaysWhyNot()
        {
            ...
            foreach (ScannedFile file in ScanTheFramework())
            {
                ...
                // ONE FILE, ONE ANSWER. This file writes Core Keeper components, so this file
                // either runs the sweep or is named in the list with a reason. A type written
                // across several files needs a row per part, and writing thirty-four of them is
                // what the file split cost.
                //
                // Bucketing the answer by type instead — the text before the first dot — does not
                // work. It makes the one row against
                // NullforgeDimensionService.RuntimeLoadingInternals.cs answer for all
                // seventy-three files of that type, the row against DimensionObjectSpine.cs answer
                // for seventeen, and it lets one sweeping part cover its siblings that write
                // components and never sweep: three files of the creature generator, two of the
                // item generator, two of the world-object generator and one of the plant
                // generator go unquestioned.
