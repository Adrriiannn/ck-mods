...
namespace ExpandNullforge.EditorTools
{
    ...
    internal sealed class DimensionObjectLinkTests
    {
        ...
        {
            // THE TWO CREATION CALLS DO NOT BELONG IN RegisterClientWorld AT ALL — not above its
            // "already registered" early-out, and not below it either. Everything that touches a
            // serialized submap is server-side —
            // DeserializeComponentsSystem is [WorldSystemFilter(ServerSimulation)] — so a client
            // copy of either system queries and returns for ever, and EnsureSystemOrdering asks
            // a client group to order the capture system against a system that world does
            // not have. Both classes say ServerSimulation, and the client block must not
            // contradict them.
            string entry = File.ReadAllText(ModEntryPath());
            string server = MethodBody(entry, "RegisterServerWorld");
            string client = MethodBody(entry, "RegisterClientWorld");
        ...
        public void TheConvertedGeneratorsNoLongerParseObjectNamesThemselves()
        {
            ...
            string[] files =
            {
                "Editor/Generators/Objects/DimensionItemGenerator.cs",
                "Editor/Generators/Objects/DimensionProjectileGenerator.cs",
                "Editor/Generators/Objects/DimensionExplosionGenerator.cs",
        ...
        public void EveryLinkInTheEnumHasSomethingThatEmitsIt()
        {
            ...
            // the strength of a sentence that merely names it.
        ...
        public void TheEnumSaysNothingItCannotDo()
        {
            ...
            // learn what a member costs. A remark that says every member is wired while fourteen
            // are not is the failure, so it is pinned to the claim the test above actually checks.
            string enumSource =
        ...
        {
            // Rule (3) of the hydration table's own remarks. Every "Tick 'x'" in it has to name a
            // control a modder can find — the shot's control is a dropdown, the scanner's is a
            // text box, and "it fires a beam" is a heading rather than a tick.
            //
            // IT READS THE MESSAGES RATHER THAN A LIST OF LITERALS. Nine hard-coded strings stay
            // green through every control invented after they are written — the same criticism
            // this wave made of the
            // enum census. This lifts every quoted phrase out of the repair messages and looks
        ...
        /// <remarks>
        /// Resolving from Directory.GetCurrentDirectory() gives whatever launched
        /// the editor rather than the project folder, so on a machine started from anywhere else
        /// the read finds nothing.
        /// </remarks>
