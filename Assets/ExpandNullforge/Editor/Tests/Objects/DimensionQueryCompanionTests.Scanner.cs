...
namespace ExpandNullforge.EditorTools
{
    ...
    internal sealed partial class DimensionQueryCompanionTests
    {
        ...
        /// <remarks>
        /// <para>
        /// WHY NOT LINE BY LINE. <c>ReadAllLines</c> plus a per-line
        /// regex misses a generic call split across two lines —
        /// <c>EnsureComponent&lt;</c> on one line and the type on the next is invisible to all
        /// three scans. The tree already wraps generic calls at the margin, so one long type name on an
        /// add would take that component out of the guard permanently, and out of the backlog
        ...
        /// and <c>DimensionDungeonAssembler.cs</c> — so "nothing in those folders writes one" is
        /// false, and widening the scan is not free: it
        /// is what puts those four on the excused list with a reason. This test's own folder is
