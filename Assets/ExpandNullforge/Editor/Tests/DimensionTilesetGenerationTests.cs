...
namespace ExpandNullforge.EditorTests
{
    ...
    /// </para>
    /// <para>
    /// A MISSING CAPTURE IS A FAILURE, NOT A SKIP. Every precondition here used to call
    /// <c>Assert.Ignore</c>, so on a machine with no captured tables and no shipped sheets all eight
    /// tests reported Ignored and the suite read green — the project's strongest claim proving nothing
    /// while looking proven. The preconditions now fail and say what to fetch.
    /// </para>
    /// </remarks>
    public sealed class DimensionTilesetGenerationTests
    {
        ...
        /// </summary>
        /// <remarks>
        /// Walls used to reproduce only 4 of the 24 — the quarter recipe had been mined from Dirt's own
        /// bake, and Dirt turns out to be the outlier: 20 other tilesets keep some wall quarters at
        /// different places in their sheets. The recipe is now chosen per slot to satisfy every tileset
