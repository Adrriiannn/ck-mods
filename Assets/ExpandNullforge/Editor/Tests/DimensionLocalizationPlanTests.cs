...
namespace ExpandNullforge.EditorTests
{
    ...
    /// </summary>
    /// <remarks>
    /// Every generator but the item one used to write a prefab and no localization row, so the
    /// player read the key — "Items/MyMod_stoneBench" — in the tooltip where the name belonged.
    ...
    public sealed class DimensionLocalizationPlanTests
    {
        ...
        /// <summary>
        /// The whole point: each family that used to reach the game nameless now carries a name and
        /// a tooltip line.
        /// </summary>
        [Test]
        ...
        /// <summary>
        /// A vehicle is named like every other placeable, because it now IS one.
        /// </summary>
        /// <remarks>
        /// This test used to assert the opposite, and was right to: a vehicle was generated as
        /// <c>ObjectType.NonObtainable</c> with no placement, so it never appeared in a slot and a row
        /// for it would have been a key nothing reads. A vehicle is <c>PlaceablePrefab</c> with an
        /// icon and placement now, exactly as the game's own Boat and Minecart are, so the honest
        /// answer flipped with it.
        /// </remarks>
        [Test]
