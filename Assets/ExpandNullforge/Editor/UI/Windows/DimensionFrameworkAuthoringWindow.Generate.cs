...
namespace ExpandNullforge.EditorTools
{
    ...
    public sealed partial class DimensionFrameworkAuthoringWindow
    {
        ...
        /// </para>
        /// <para>
        /// ONLY THE RULES THAT REALLY ASK FOR SOMETHING. Walking <c>RequiredItems</c> on
        /// every enabled rule catches lists kept after a creator switches "When it opens" to a
        /// mode that never reads them — <c>AppendTravelRequirements</c> returns immediately unless
        /// <c>UsesRequiredItems</c>, and the Access page hides the offering card entirely — so
        /// generating after that switch would warn about entries the creator can neither see on the
            ...
            List<string> intoReportWarnings)
        {
            ...
            for (int i = 0; i < rules.Length; i++)
            {
                ...
                for (int n = 0; n < items.Length; n++)
                {
                    ...
                    {
                        // IT DOES NOT HOLD THE DOOR SHUT, whatever an empty slot looks like.
