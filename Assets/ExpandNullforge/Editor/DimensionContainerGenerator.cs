...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static class DimensionContainerGenerator
    {
            ...
            DimensionDropPlan plan)
        {
            ...
            // reloads the prefab it wrote last time, so an author who deleted every drop off this
            // chest used to get last time's drops baked on it for ever, and a renamed mod left it
            // pointing at a table minted from the old name that the runtime never fills.
            ...
            DimensionContainerGenerationReport report)
        {
            ...
            for (int i = 0; i < rules.Length; i++)
            {
                ...
                for (int o = 0; o < items.Length; o++)
                {
                    ...
                    // the same place now.
                    //
                    // COUNTED, NOT PREDICTED. The warning used to sit above this loop and fire on
                    // how many names were TYPED, so nine names of which three were misspellings
                    // lost nothing to the cap and the author was still told the game had dropped
                    // some. Only names that would really have gone in are counted here.
        ...
        /// </para>
        /// <para>
        /// The runtime lookup used to be tried afterwards as well, on the theory that a name the mod
        /// owns exists only there. It does not exist there either at generation time — that
        /// dictionary is empty — so the call could only ever answer <c>None</c> while looking as
