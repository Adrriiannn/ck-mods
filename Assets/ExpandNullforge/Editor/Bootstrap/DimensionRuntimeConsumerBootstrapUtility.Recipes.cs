...
namespace ExpandNullforge.EditorTools
{
    ...
    internal static partial class DimensionRuntimeConsumerBootstrapUtility
    {
        ...
        /// </para>
        /// <para>
        /// AND WHY BY NAME RATHER THAN BAKED. Writing the output into
        /// <c>CraftingAuthoring.moddedObjectID</c> has Core Keeper resolve it DURING conversion
        ...
        /// converts before or after the item it makes is a coin flip that changes when a
        /// prefab is renamed. Lose it and the bench bakes <c>ObjectID.None</c> into its recipe
            ...
            string modName)
        {
            ...
            for (int w = 0; workbenches != null && w < workbenches.Length; w++)
            {
                ...
                // A BENCH THAT BORROWS A STATION HAS NO PREFAB TO BAKE ITS RECIPES ONTO, so it must
                // not be skipped here as well — skipped, a workbench that names an existing station,
                // which DimensionWorkbenchAsset documents as supported, registers its recipes
                // nowhere, is warned about nowhere, and appears nowhere. Its station is the object
                // it named, resolved at load like any other name.
