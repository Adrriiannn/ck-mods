...
namespace ExpandNullforge.Api
{
    ...
    public static class DimensionCapabilityRegistry
    {
        private static readonly DimensionCapability[] Capabilities =
        {
            ...
            new DimensionCapability(
                "world-rules-armour-sets",
                "Armour sets and their bonuses",
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "A mod's own armour sets reach the game through a prefix on SummarizeConditionsSystem.OnCreate, which is where Core Keeper loads SetBonusesTable by name and bakes it. Rows are appended at the END, because the bake that follows re-rolls every set's numbers from a stream seeded at 1337 and walked in list order, so anything inserted earlier would quietly change all 62 of the game's own sets. Set numbers come from the set ids in name order, past the highest SetBonusID the game has (measured: DreamerSet, 62, so 63 is the first free). A piece may belong to one set only and a clash is refused with a line, because both lookups the game builds are keyed by the item and throw on a repeat. Honest limits: the value on a bonus line is not authored — the game computes it from the tier and the rarity and overwrites whatever is stored; and the hover panel has six text slots for pieces and six for lines, so a bigger set works and is not fully listed. The name ordering, the piece clash, append-once-per-session and the zero value are test-locked and executed outside Unity; in-game evidence pending."),
            new DimensionCapability(
                "world-rules-backgrounds",
                "What a background starts you with",
                DimensionCapabilityMaturity.PartialVerticalSlice,
                "The starting skill and starter kit of any of Core Keeper's eleven backgrounds are answered through a postfix on RolePerksTable.GetPerks, the one accessor both readers go through — the player granting the kit on a character's first play, and the character-creation screen previewing it. The game's asset is never written to. A TWELFTH BACKGROUND IS NOT POSSIBLE and is not offered: the creation carousel sizes itself from Enum.GetNames(typeof(CharacterRole)).Length, steps modulo that number, and indexes an eleven-entry picture list by position. Two starter items is a hard ceiling, measured on the character-creation prefab: the screen writes each item into roleItemDescs[i] with no bounds check and that list has two slots. A third is dropped with a line rather than shipped. Nomad's starting skill is skipped by a hardcoded test on the background number, so naming one for Nomad is reported. A row edits only the halves it fills in: leaving the skill blank keeps the skill the game starts that background in, and leaving the bag empty keeps the game's bag — the answer is built on top of Core Keeper's own rather than from nothing, because it replaces the whole Perks struct and building from nothing moved every part-edited background to Mining with an empty bag. The ceiling, the answer-for-what-was-claimed rule, the keep-the-half-you-left-blank rule and the retry-until-names-resolve rule are test-locked and executed outside Unity; in-game evidence pending."),
            new DimensionCapability(
                "world-rules-environment-events",
                "When the world acts on its own",
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "Where and when cave-ins, larva swarms, bomb-scarab swarms and Omoroth's tentacles happen is rewritten through a prefix on EnvironmentEventSystem.OnCreate, which loads EnvironmentEventsTable by name and bakes it into native containers for the life of the world; the original list is put back in the postfix. The requirements are the whole of what decides an event: which of the game's biomes, distance from the core, how many of which tiles are around the player, whether a boss is near, and the cooldowns. Custom blocks CAN be counted in the tile requirements, because that check compares the tile's own tileset number. Honest limits, both measured rather than assumed: only Core Keeper's OWN biomes can be named, because the biome the check compares against comes from BiomeLookup.GetBiome, which reads a byte per sampled tile or a fixed list indexed by the biome and nothing in this framework writes either — a custom biome is refused at generate time with a line rather than shipped as a rule that never matches. The refusal compares the name against a written list of the nine biomes a player can stand in rather than parsing it, because Enum.TryParse takes a number as readily as a name — a biome id written as 1000 used to walk straight past the refusal, and so did Obsidian and GreatWall, which Core Keeper marks obsolete and its generator never lays. And there are FOUR events and there can only be four: the behaviour of each is a set of Burst-compiled function pointers written out one by one in the same OnCreate, keyed 1 to 4. An event named twice keeps the last, because the game bakes into a hash map keyed by the event and that add throws on a repeat. One-row-per-event, switched-off-means-no-row, untouched-events-keep-their-rules and the biome refusal are test-locked, and the first three are executed outside Unity; in-game evidence pending."),
            new DimensionCapability(
                "world-rules-games-own-terrain",
                "Your blocks in Core Keeper's own caves",
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "The only door into the game's own procedurally generated ground. Rules are added to TileTypeMapping through a prefix on SpawnProceduralTerrainSystem.OnCreate, which loads it by name and copies it into a persistent NativeArray; the original list is put back in the postfix. A rule asks what the generator decided about a tile — biome, material, open floor, roof hole, Great Wall, resource slot — and answers with one part of one block, which may be a tileset of this mod's own. This is what puts a custom ore into a vanilla biome: measured over the game's own 75 rules in Assets/Resources/TileTypeMapping.asset, 21 of them are exactly that (15 lay ore, 6 lay ancient crystal, and those 21 are also the only rules naming a resource slot), and the Dirt biome uses two of five resource slots while Crystal and Excavation use one. Rules go at the FRONT of the list, because the generator pushes matching rules into a NativeParallelMultiHashMap in list order and that map enumerates a key's values in reverse, so the earliest matching rule is written last and wins. Honest limits: the question half is the generator's own fixed vocabulary of ten biomes and twenty-one materials, so a biome this mod added cannot be asked about; and one such table means a rule applies to the whole world, not to one dimension. The front-of-list law and the full rule shape are test-locked and executed outside Unity; in-game evidence pending."),
            new DimensionCapability(
                "world-rules-skill-pictures",
                "Your pictures on the game's skills",
                DimensionCapabilityMaturity.PartialVerticalSlice,
                "A mod's own sprite replaces the picture on one of Core Keeper's twelve skills, supplied through a postfix on SkillIconsTable.GetIcon rather than written into the game's asset — the same law the talent-icon patch follows, and necessary because SkillIconsTable is not loaded by name anywhere; it reaches the game as an inspector reference on SkillUIElement, its only reader. Both the ordinary and the maxed-out gold picture can be set, and giving only one keeps the game's other. Honest limits: this repaints, it does not add — a thirteenth skill is not possible — and there is one such table, so it repaints the whole game's skill window. Half-claims and empty rows are test-locked; in-game evidence pending."),
            new DimensionCapability(
                "pet-colours",
                "Colours a pet can be recoloured into",
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "A pet this mod adds can now come in colours. Core Keeper keeps pet skins in PetInfosTable rather than on the pet, and three readers ask through one method: PetConverter, which turns the count into PetCD.maxSkins as the pet converts; PetBase, which applies the chosen gradient map to the sprite; and the inventory and hover UI, which tint its icon. A mod's pet answered null there, so it has always converted with a skin count of zero. One postfix on GetPetSkinInfo answers all three. The table is not written to, and here that is not only hygiene: PetInfosTable builds its pet-skin lookup on first use and NEVER rebuilds it — OnValidate clears the talent lookup and nothing clears the skin one, not even in the editor — so a row appended after the first question would be invisible for the session with no reflection-free way to clear it. A pet id written without the mod in front of it is asked for both ways, because the object the generator wrote is registered under the qualified name. Colours are authored as GradientMapDataBlock references on the pet and addressed by the block's own address, which the loader registers for a mod's data blocks. Replace-not-stack and refuse-the-empty are test-locked; in-game evidence pending."),
            new DimensionCapability(
            ...
            // Every one below is a collection or page in Editor/UI/Shell/DimensionStageCatalog.cs, so a
            ...
            new DimensionCapability(
                ...
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "Placeable objects with authored art, placement rules, what they do when used, what they leave behind, and whether they turn to face the player — plus the wiring and automation surfaces (what needs power, what pushes, what picks up, what may reach into inventories) and traders (stock, unlocks, seasonal rules), all applied through DimensionObjectSpine by DimensionWorldObjectGenerator. An object can also give off light where it stands, which is not a component but the five-node subtree Core Keeper puts on the graphical prefab: colour, reach, brightness, flicker, shadows, height and how far off the middle of the tile it sits, seeded from six of the game's own lights measured off their prefabs, built by the same builder the dimension portal's own light has always used. It is the object's OWN light even on an object something uses, whose graphical prefab Core Keeper shares between every object of that use: every such prefab carries the light nodes and DimensionAuthoredLight re-derives the light per entity on each occupy, the same answer DimensionAuthoredBody already gives for the picture, so a lit forge and a plain bench in one mod light differently and a generated container inherits neither. Two limits are the game's and are said on the toggle a modder ticks: lights are kept one per two tiles of floor, so two on next-door tiles may not both light, and the renderer holds 512 lit lights on screen at once. In-game evidence pending."),
            new DimensionCapability(
                "layout-versioning",
