    ...
    public static class DimensionCapabilityRegistry
    {
        private static readonly DimensionCapability[] Capabilities =
        {
            ...
            new DimensionCapability(
                ...
                DimensionCapabilityMaturity.ImplementedNotFullyProven,
                "A pet this mod adds can now come in colours. Core Keeper keeps pet skins in PetInfosTable rather than on the pet, and three readers ask through one method: PetConverter, which turns the count into PetCD.maxSkins as the pet converts; PetBase, which applies the chosen gradient map to the sprite; and the inventory and hover UI, which tint its icon. A mod's pet answered null there, so it has always converted with a skin count of zero. One postfix on GetPetSkinInfo answers all three. The table is not written to, and here that is not only hygiene: PetInfosTable builds a private lookup on first use and never rebuilds it outside the editor, so a row appended after the first question would be invisible for the session with no reflection-free way to clear it. Colours are authored as GradientMapDataBlock references on the pet and addressed by the block's own address, which the loader registers for a mod's data blocks. Replace-not-stack and refuse-the-empty are test-locked; in-game evidence pending."),
            new DimensionCapability(
