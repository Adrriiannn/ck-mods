  ...
  public sealed partial class NullforgeDimensionService
  {
        ...
        DimensionContentValidationRequest request)
    {
      ...
      foreach (DimensionSpawnRule rule in spawnRules.Values)
      {
        ...
        string ownerId = GetContentValidationOwnerId(DimensionContentRecordKind.SpawnRule, rule.RuleId);
        if (!IsValidSpawnSubjectKind(rule.SubjectKind) ||
            rule.SubjectKind == DimensionSpawnSubjectKind.Any)
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              rule.RuleId,
              ownerId,
              "spawn-rule-kind-invalid",
              "The spawn rule subject kind is not supported.");
        }
      ...
      foreach (DimensionEncounterDefinition encounter in encounters.Values)
      {
        ...
        if (!string.IsNullOrEmpty(encounter.SpawnRuleId) &&
            spawnRules.TryGetValue(encounter.SpawnRuleId, out spawnRule) &&
            !string.Equals(spawnRule.DimensionId, encounter.DimensionId, StringComparison.Ordinal))
        {
          AddContentValidationIssue(
              ...
              encounter.EncounterId,
              ownerId,
              "encounter-spawn-rule-dimension-mismatch",
              "The encounter spawn rule belongs to another dimension.");
        }
        else if (!string.IsNullOrEmpty(encounter.SpawnRuleId) &&
                 !spawnRules.ContainsKey(encounter.SpawnRuleId))
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              encounter.EncounterId,
              ownerId,
              "encounter-spawn-rule-missing",
              "The encounter points at a missing spawn rule.");
        }
      ...
      foreach (DimensionResourceNodeDefinition node in resourceNodes.Values)
      {
        ...
        string ownerId = GetContentValidationOwnerId(DimensionContentRecordKind.ResourceNode, node.NodeId);
        if (!IsValidResourceNodeKind(node.Kind) ||
            node.Kind == DimensionResourceNodeKind.Any)
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              node.NodeId,
              ownerId,
              "resource-node-kind-invalid",
              "The resource node kind is not supported.");
        }
        ...
        if (!string.IsNullOrEmpty(node.GenerationPassId) &&
            generationPasses.TryGetValue(node.GenerationPassId, out generationPass) &&
            !string.Equals(generationPass.DimensionId, node.DimensionId, StringComparison.Ordinal))
        {
          AddContentValidationIssue(
              ...
              node.NodeId,
              ownerId,
              "resource-node-pass-dimension-mismatch",
              "The resource node generation pass belongs to another dimension.");
        }
        else if (!string.IsNullOrEmpty(node.GenerationPassId) &&
                 !generationPasses.ContainsKey(node.GenerationPassId))
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              node.NodeId,
              ownerId,
              "resource-node-pass-missing",
              "The resource node points at a missing generation pass.");
        }
      ...
      foreach (DimensionEnvironmentProfile profile in environmentProfiles.Values)
      {
        ...
        if (!definitions.ContainsKey(profile.DimensionId))
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              DimensionContentRecordKind.EnvironmentProfile,
              profile.ProfileId,
              GetContentValidationOwnerId(DimensionContentRecordKind.EnvironmentProfile, profile.ProfileId),
              "environment-profile-dimension-missing",
              "The environment profile points at a missing dimension.");
        }
      }
      ...
      foreach (DimensionBiomeDefinition biome in biomes.Values)
      {
        ...
        if (!string.IsNullOrEmpty(biome.EnvironmentProfileId))
        {
          ...
          if (!environmentProfiles.TryGetValue(biome.EnvironmentProfileId, out profile))
          {
            AddContentValidationIssue(
                issues,
                request,
                ...
                biome.BiomeId,
                ownerId,
                "biome-environment-profile-missing",
                "The biome points at a missing environment profile.");
          }
          else if (!profile.Enabled)
          {
            AddContentValidationIssue(
                ...
                biome.BiomeId,
                ownerId,
                "biome-environment-profile-disabled",
                "The biome points at a disabled environment profile.");
          }
        }
      }
      ...
      foreach (DimensionGenerationTableDefinition table in generationTables.Values)
      {
        ...
        string ownerId = GetContentValidationOwnerId(DimensionContentRecordKind.GenerationTable, table.TableId);
            ...
            !definitions.ContainsKey(table.DimensionId))
        {
          AddContentValidationIssue(
              ...
              table.TableId,
              ownerId,
              "generation-table-dimension-missing",
              "The generation table points at a missing dimension.");
        }

        if (!string.IsNullOrEmpty(table.BiomeId))
        {
          DimensionBiomeDefinition biome;
          if (!biomes.TryGetValue(table.BiomeId, out biome))
          {
            AddContentValidationIssue(
                issues,
                request,
                ...
                "The generation table points at a missing biome.");
          }
          else if (!biome.Enabled)
          {
            AddContentValidationIssue(
                issues,
                request,
                ...
                table.TableId,
                ownerId,
                "generation-table-biome-disabled",
                "The generation table points at a disabled biome.");
          }
        }
      }
      ...
      foreach (DimensionGenerationTableEntryDefinition entry in generationTableEntries.Values)
      {
        ...
        string ownerId = GetContentValidationOwnerId(DimensionContentRecordKind.GenerationTableEntry, entry.EntryId);
        DimensionGenerationTableDefinition table;
        if (!generationTables.TryGetValue(entry.TableId, out table))
        {
          AddContentValidationIssue(
              issues,
              request,
              ...
              entry.EntryId,
              ownerId,
              "generation-table-entry-parent-missing",
              "The generation table entry points at a missing parent table.");
        }
        else if (!table.Enabled)
        {
          AddContentValidationIssue(
              ...
              entry.EntryId,
              ownerId,
              "generation-table-entry-parent-disabled",
              "The generation table entry points at a disabled parent table.");
        }
      }
