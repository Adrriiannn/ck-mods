    ...
    public static class DimensionFrameworkSetupGuideUtility
    {
        public static DimensionFrameworkSetupGuide Build(DimensionTemplateAuthoringWorkspace workspace)
        {
            ...
            AddStep(
                ...
                "Defines biome identity, fallback bounds, semantic terrain IDs, environment/palette/generation links, scenes, resources, and spawns.",
            ...
            AddStep(
                steps,
                "visual-palette",
                "Reference visual assets through a palette",
                graph != null && graph.Palette != null && VisualReportHasNoBlockedRefs(visualReport),
                "BiomePaletteTemplateAsset",
                "Assets/<YourDimension>/Authoring/Palettes/<BiomeName>Palette.asset",
                "Use vanilla resource keys when you want vanilla assets to track future game updates, or mod asset references when you provide custom visuals.",
                "Visual preflight should report no blocked references; UGC SpriteObject Lit/runtime-swap review notes are advisory until the prefab path is proven.",
                "create-palette-template");

            AddStep(
                steps,
                "environment-profile",
                "Configure environment presentation",
                graph != null && graph.EnvironmentProfile != null,
                "EnvironmentProfileTemplateAsset",
                "Assets/<YourDimension>/Authoring/Environment/<BiomeName>Environment.asset",
                "Captures map color, ambient profile, lighting/fog/music/zone defaults, and other environment-facing biome hints.",
                "Environment readiness should be ready or advisory-only; missing environment data should not block early prototyping.",
            ...
            AddStep(
                steps,
                "generation-tables",
                "Add weighted generation tables or semantic terrain IDs",
                assessment != null && assessment.HasGenerationTable,
                "GenerationTableTemplateAsset",
                "Assets/<YourDimension>/Authoring/Generation/Tables/*.asset",
                "Weighted tables drive terrain, floors, walls, liquids, ores, objects, scenes, resources, mobs, bosses, and custom provider choices.",
                "Tables should preview probabilities cleanly; explicit tables override shorthand semantic terrain tables when IDs collide.",
