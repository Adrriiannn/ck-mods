...
namespace ExpandNullforge.Authoring
{
    ...
    public static class DimensionTemplateCustomizerNavigationUtility
    {
            ...
            DimensionTemplateAuthoringWorkspace workspace)
        {
            ...
            int count = CountArray(template.GlobalResourceNodes) +
                CountArray(template.GlobalItems) +
            ...
            for (int i = 0; i < biomes.Length; i++)
            {
                ...
                if (biome != null)
                {
                    count += CountArray(biome.ResourceNodes);
                }
            }
            ...
            DimensionTemplateAuthoringWorkspace workspace)
        {
            ...
            int count = CountArray(template.GlobalAnimals) +
                CountArray(template.GlobalCritters) +
                CountArray(template.GlobalMobs) +
                CountArray(template.GlobalBosses) +
                CountArray(template.GlobalSpawnRules) +
                CountSceneSpawnPoints(template.GlobalScenes);
            BiomeTemplateAsset[] biomes = template.Biomes;
            for (int i = 0; i < biomes.Length; i++)
            {
                BiomeTemplateAsset biome = biomes[i];
                if (biome != null)
                {
                    count += CountArray(biome.SpawnRules) +
        ...
        private static int CountSceneSpawnPoints(SceneTemplateAsset[] scenes)
        {
            ...
            for (int i = 0; i < scenes.Length; i++)
            {
                ...
                if (scene != null)
                {
                    count += CountArray(scene.SpawnPoints);
                }
            }
            ...
            DimensionAuthoringPreviewSummary preview)
        {
            return
                ...
                CountContent(preview, DimensionAuthoringContentSummaryKind.BiomeGenerationProfile);
        }
