...
namespace ExpandNullforge.Authoring
{
    ...
    public sealed class DimensionItemAsset : ScriptableObject
    {
        ...
        /// <remarks>
        /// One number, two fields, and until now the wrong one won. The effects block used to carry
        /// a second seconds-between-uses of its own, and because the effects pass ran after this
        /// one it destroyed the cooldown component whenever its own field was blank — so the field
        /// a creator could see set neither the swing rate nor the durability divisor, and every
        /// generated melee weapon came out at the game's default. The field on the item is now the
        /// only one drawn, and the old one is read here so an asset that stored its number there
        /// keeps it.
        /// </remarks>
